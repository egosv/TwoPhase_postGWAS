...
savedir <- paste0(setwd,"results")

library(data.table)

### Load files ----
resOpt_tot <- data.frame()
for (file in grep(paste0("NFBC66_TG_twophase_.+[.]RData"),dir(),value=T) ) { 
...
uniq_val <- function(col){
  tmp <- tabres1[,col]
  for(i in 2:length(tmp)){
    if(tabres1[i,col]==tabres1[i-1,col]) tmp[i] <- NA
  }
  return( tmp)
}
tabres1$ss <- uniq_val("ss")
tabres1$GENE <- uniq_val("GENE")
tabres1$chromosome <- uniq_val("chromosome")
...
###* Beta-Beta plots ----
{
  ...
  ggsave(bb_plot, file=paste0(savedir,"/App_beta-betaplot.png"), width=11, height=8.5, units="in", bg="transparent", dpi=500)
...
###* Region plots ----
{
  ...
  ggsave(regplot, file=paste0(savedir,"/RegionPlot_App.png"), width=8, height=4, units="in", bg="transparent", dpi=500)
...
png(paste0(savedir,"/Mosaics_App_ph1.png"),width = 7, height = 4, units = "in", bg="transparent", res=300)
grid.draw(p0)
dev.off()

### Plot comparing designs across ph2 sample sizes
p1 = grid.grabExpr(mosaic(StrataZ~S|design, data=xtabs(R ~ StrataZ + S + design, data=subset(df.mosaic,ss==449)), labeling_args=vnames0, set_labels=lnames, zero_size = 0, main = "449", margins=unit(c(2.5,3,0.5,3), "lines"), highlighting_fill = myColors, main_gp = gpar(fontsize = 15)))
p2 = grid.grabExpr(mosaic(StrataZ~S|design, data=xtabs(R ~ StrataZ + S + design, data=subset(df.mosaic,ss==1123)), labeling_args=vnames0, set_labels=lnames, zero_size = 0, main = "1123", margins=unit(c(2.5,3,0.5,3), "lines"), highlighting_fill = myColors, main_gp = gpar(fontsize = 15)))
p3 = grid.grabExpr(mosaic(StrataZ~S|design, data=xtabs(R ~ StrataZ + S + design, data=subset(df.mosaic,ss==2246)), labeling_args=vnames, set_labels=lnames, zero_size = 0, main = "2246", margins=unit(c(2.5,3,0.5,3), "lines"), highlighting_fill = myColors, main_gp = gpar(fontsize = 15)))


obj <- grid.arrange(p1, p2, p3, ncol=3)
png(paste0(savedir,"/Mosaics_App.png"),width = 7, height = 4, units = "in", bg="transparent", res=300)
grid.draw(obj)
dev.off()
